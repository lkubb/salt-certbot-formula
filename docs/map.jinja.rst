...
We will see a quick setup to configure the ``certbot`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``certbot`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``certbot`` formulas under ``/srv/salt/certbot/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/certbot/parameters/dns:domain/
    ln -s dns:domain /srv/salt/certbot/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/certbot/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/certbot/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``certbot`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``certbot`` formulas under ``/srv/salt/certbot/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/certbot/parameters/roles
...
We create a configuration for the role ``certbot/server`` in ``/srv/salt/certbot/parameters/roles/certbot/server.yaml``:
...
We create another configuration for the role ``certbot/client`` in ``/srv/salt/certbot/parameters/roles/certbot/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles certbot/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles certbot/client
...
.. code-block:: console
    ...
    mv /srv/salt/certbot/parameters/dns:domain/  '/srv/salt/certbot/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-certbot-as-json:
      test.nop:
        - name: {{ certbot | json }}
...
Here is an example based on `template-formula/certbot/config/file.sls`_:
...
This ``sls`` file expose a ``certbot`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ certbot['winner'] }}
...
.. _template-formula/certbot/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/certbot/config/file.sls
